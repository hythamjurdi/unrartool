...
- **Fix: expanded defer detection** — catches more "still downloading" error patterns from unrar so jobs defer instead of hard-failing
- **Fix: webhook path resolution** — all parsers now try `downloadFolder` first before media library paths; all candidates verified to exist on disk
